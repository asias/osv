...
struct page_buffer {
    page_buffer(sched::cpu* cpu)
        : _fill_thread([] { fill_thread(); },
            sched::thread::attr().pin(cpu).name(osv::sprintf("page_buffer%d", cpu->id)))
    {
        _fill_thread.start();
    }
    ...
    void* pop() { assert(nr != 0); return _pages[--nr]; }
    void push(void* page) { _pages[nr++] = page; assert(nr <= max); }
    void* top() { assert(nr != 0); return _pages[nr - 1]; }
    ...
    static constexpr size_t pages_per_batch = 32;
...
struct page_batch {
    void* pages[page_buffer::pages_per_batch];
};
...
class page_buffer_l2 {
public:
    page_buffer_l2()
        : _max(sched::cpus.size() * (page_buffer::max / page_buffer::pages_per_batch))
        , _nr(0)
        , _watermark_lo(_max * 1 / 4)
        , _watermark_hi(_max * 3 / 4)
        , _stack(_max)
        , _fill_thread([=] { fill_thread(); }, sched::thread::attr().name("page_buffer_l2"))
    {
       _fill_thread.start();
    }
    ...
    bool free_page_batch(page_batch* pb)
    {
...
static sched::cpu::notifier _notifier([] () {
    *percpu_page_buffer = new page_buffer(sched::cpu::current());
...
static inline page_buffer& get_page_buffer()
{
    return **percpu_page_buffer;
}

class page_buffer_l2 global_page_buffer_l2;

// Percpu thread for L1 page pool
void page_buffer::fill_thread()
{
    sched::thread::wait_until([] {return smp_allocator;});
    auto& pbuf = get_page_buffer();
    for (;;) {
...
void page_buffer::refill()
{
    WITH_LOCK(preempt_lock) {
        page_batch* pb;
        auto& pbuf = get_page_buffer();
        while (pbuf.nr + page_buffer::pages_per_batch < pbuf.max / 2) {
            while (!(pb = global_page_buffer_l2.alloc_page_batch())) {
                WITH_LOCK(migration_lock) {
                    DROP_LOCK(preempt_lock) {
                        global_page_buffer_l2.refill();
                    }
                }
                // Check again since someone else might change pbuf.nr when we sleep
                if (!(pbuf.nr + page_buffer::pages_per_batch < pbuf.max / 2))
                    return;
            }
...
void page_buffer::unfill()
{
    WITH_LOCK(preempt_lock) {
        auto& pbuf = get_page_buffer();
        while (pbuf.nr - page_buffer::pages_per_batch > pbuf.max / 2) {
            auto* pb = static_cast<page_batch*>(pbuf.top());
            for (size_t i = 0 ; i < page_buffer::pages_per_batch; i++) {
                pb->pages[i] = pbuf.pop();
            }
            while (!global_page_buffer_l2.free_page_batch(pb)) {
                WITH_LOCK(migration_lock) {
                    DROP_LOCK(preempt_lock) {
                        global_page_buffer_l2.unfill();
                    }
                }
            }
        }
...
void* page_buffer::alloc_page_local()
{
...
bool page_buffer::free_page_local(void* v)
{
    SCOPE_LOCK(preempt_lock);
    auto& pbuf = get_page_buffer();
    if (pbuf.nr > pbuf.watermark_hi) {
...
void page_buffer_l2::fill_thread()
{
    if (smp_allocator_cnt++ == sched::cpus.size()) {
        smp_allocator = true;
    }
...
void page_buffer_l2::refill()
{
    ...
    while (get_nr() < _max / 2) {
        ...
        pb = static_cast<page_batch*>(batch.pages[page_buffer::pages_per_batch - 1]);
        *pb = batch;
        if (_stack.push(pb)) {
            inc_nr();
        } else {
...
void page_buffer_l2::free_batch(page_batch& batch)
{
    WITH_LOCK(free_page_ranges_lock) {
        for (size_t i = 0 ; i < page_buffer::pages_per_batch; i++) {
